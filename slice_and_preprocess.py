...
for i, train_filename in enumerate(train_filenames):
    ...
    mean, std, _min, _max, saved_df = fn.save_slices(
        train_filename.name, i, tiff, mask, roi_mask, savepath, saved_df, **conf)
    means.append(mean)
    stds.append(std)
    mins.append(_min)
    maxs.append(_max)
...
np.save(conf.out_dir + "normalize_train",
        np.asarray((means, stds, mins, maxs)))
...
for i, val_filename in enumerate(val_filenames[10:]):
    ...
    roi_mask = fn.get_mask(tiff, roi_shp, column="CONTINENT")
    mean, std, _min, _max, saved_df = fn.save_slices(
        val_filename.name, i, tiff, mask, roi_mask, savepath, saved_df, **conf)
    means.append(mean)
    stds.append(std)
    mins.append(_min)
    maxs.append(_max)
